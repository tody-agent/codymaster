...
🦴 **Smart Spine (v4.6+)** — 모든 5계층을 연결하는 신경 시스템:
...
- **MCP Server** — Claude Desktop 및 모든 MCP 클라이언트용 **13**개 도구 (`src/mcp-context-server.ts`).
...
📖 [CodyMaster Brain & 메모리 모델 →](docs/architecture/codymaster-brain.md)
...
📖 [TRIZ 병렬 워크플로 & UI 프리뷰 →](docs/architecture/triz-parallel-engine.md)
...
| 🎯 **오케스트레이션** | `cm-execution` `cm-continuity` `cm-deep-search` `cm-codeintell` `cm-how-it-work` |
...
전체 CLI 목록은 **`cm --help`**로 확인하세요 (`src/cli/command-registry.ts`와 동일).

**CLI 명령어:**
```
cm, codymaster              → CLI 진입점
cm status / task / project / deploy / dashboard
cm agent / brain            → 작업 메모리·Continuity
cm chain                    → Skill chain
cm browse / guardian / sprint / design-studio / distro validate
# MCP·cm://·bus: docs/api/api-reference.md
# 엔지니어링: docs/workflows/engineering-pipeline.md
```
...
- 📖 [문서 (사이트)](https://cody.todyle.com/docs)
- 📘 [문서 (저장소)](docs/index.md) — Markdown 원본; `npm run docs:dev` (VitePress)
- 🛠️ [스킬 레퍼런스](skills/) — 이 저장소의 **56**개 `cm-*` SKILL.md (설치 프로필로 추가 가능)
...
푸시와 PR마다 CI에서 `npm ci`와 `npm run test:gate:kit`을 실행합니다 (`.github/workflows/ci.yml`).

---

<div align="center">

*ISC 라이선스 — 자유롭게 사용, 수정 및 배포 가능합니다.* <br/>
