...
class DesignSystemGenerator:
    ...
    def _get_ux_laws(self, product_type: str) -> list:
        """Get applicable UX laws for a product type."""
        result = search(product_type, "ux-laws", max_results=10)
        return result.get("results", [])

    def _get_design_tests(self, product_type: str) -> list:
        """Get applicable design tests for a product type."""
        result = search(product_type, "design-tests", max_results=10)
        return result.get("results", [])
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        # Get UX Laws and Design Tests for this category
        ux_laws = self._get_ux_laws(category)
        design_tests = self._get_design_tests(category)
...
def format_ascii_box(design_system: dict) -> str:
    ...
    # UX Laws section
    ux_laws = design_system.get("ux_laws", [])
    if ux_laws:
        lines.append("")
        lines.append(f"{'=' * BOX_WIDTH}")
        lines.append(f"  📐 UX LAWS (Applicable)")
        lines.append(f"{'─' * BOX_WIDTH}")
        for law in ux_laws[:5]:
            law_name = law.get("Law_Name", "")
            severity = law.get("Severity", "")
            anti = law.get("Anti_Pattern", "")
            lines.append(f"  ⚖️  {law_name} [{severity}]")
            if anti:
                lines.append(f"      ⚠️  Anti-pattern: {anti[:70]}")
        lines.append(f"{'=' * BOX_WIDTH}")

    # Design Tests section
    design_tests = design_system.get("design_tests", [])
    if design_tests:
        lines.append("")
        lines.append(f"{'=' * BOX_WIDTH}")
        lines.append(f"  🧪 DESIGN TESTS (TDD for Design)")
        lines.append(f"{'─' * BOX_WIDTH}")
        for test in design_tests[:5]:
            test_id = test.get("Test_ID", "")
            assertion = test.get("Assertion", "")
            severity = test.get("Severity", "")
            lines.append(f"  [{test_id}] {assertion[:60]} [{severity}]")
        lines.append(f"{'=' * BOX_WIDTH}")
...
def format_markdown(design_system: dict) -> str:
    ...
    ux_laws = design_system.get("ux_laws", [])
    if ux_laws:
        lines.append("### 📐 UX Laws (Applicable)")
        lines.append("")
        lines.append("| Law | Severity | Anti-Pattern |")
        lines.append("|-----|----------|--------------|")
        for law in ux_laws[:5]:
            law_name = law.get("Law_Name", "")
            severity = law.get("Severity", "")
            anti = law.get("Anti_Pattern", "")
            lines.append(f"| {law_name} | {severity} | {anti} |")
        lines.append("")

    # Design Tests section
    design_tests = design_system.get("design_tests", [])
    if design_tests:
        lines.append("### 🧪 Design Tests (TDD for Design)")
        lines.append("")
        lines.append("| Test ID | Assertion | Severity |")
        lines.append("|---------|-----------|----------|")
        for test in design_tests[:5]:
            test_id = test.get("Test_ID", "")
            assertion = test.get("Assertion", "")
            severity = test.get("Severity", "")
            lines.append(f"| {test_id} | {assertion} | {severity} |")
        lines.append("")
