...
exports.antigravity = {
    ...
    emoji: '✦',
    detect() {
        if (fs.existsSync(path.join((0, paths_1.homeDir)(), '.gemini'))) {
    ...
    install(opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const target = path.join((0, paths_1.homeDir)(), '.gemini/antigravity/skills');
...
function ensureGeminiMdHint() {
    const file = path.join((0, paths_1.homeDir)(), '.gemini/GEMINI.md');
    fs.mkdirSync(path.dirname(file), { recursive: true });
    let body = '';
    if (fs.existsSync(file))
        body = fs.readFileSync(file, 'utf-8');
    if (body.includes(HINT_LINE))
        return;
