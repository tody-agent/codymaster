    ...
    container.innerHTML = personas.map((p, i) => `
      <article class="persona-card reveal" style="--delay:${i * 80}ms">
        <div class="persona-card__header">
          <span class="persona-card__emoji">${p.emoji}</span>
          <div>
            <h3 class="persona-card__role">${p.role}</h3>
            <p class="persona-card__tagline">${p.tagline}</p>
          </div>
        </div>
        <p class="persona-card__demo"><strong>Who:</strong> ${p.demographics}</p>
        <div class="persona-card__pains">
          <h4>😤 Pain Points</h4>
          <ul>${p.painPoints.map(pp => `<li>${pp}</li>`).join('')}</ul>
        </div>
  ...
  function renderJtbd() {
    ...
    container.innerHTML = jobs.map((j, i) => `
      <article class="jtbd-card reveal" style="--delay:${i * 100}ms">
        <div class="jtbd-card__number">${j.number}</div>
        <div class="jtbd-card__job">
          <p class="jtbd-card__situation">${j.situation}</p>
          <p class="jtbd-card__motivation">${j.motivation}</p>
          <p class="jtbd-card__outcome">${j.outcome}</p>
        </div>
        <div class="jtbd-card__dimensions">
          <div class="jtbd-dim"><span class="jtbd-dim__icon">⚙️</span><span class="jtbd-dim__label">Functional</span><p>${j.functional}</p></div>
          <div class="jtbd-dim"><span class="jtbd-dim__icon">💙</span><span class="jtbd-dim__label">Emotional</span><p>${j.emotional}</p></div>
          <div class="jtbd-dim"><span class="jtbd-dim__icon">👥</span><span class="jtbd-dim__label">Social</span><p>${j.social}</p></div>
        </div>
        <div class="jtbd-card__forces">
          <div class="force force--push"><span class="force__label">⬆️ ${labels.push}</span><p>${j.push}</p></div>
          <div class="force force--pull"><span class="force__label">⬇️ ${labels.pull}</span><p>${j.pull}</p></div>
          <div class="force force--habit"><span class="force__label">🔄 ${labels.habit}</span><p>${j.habit}</p></div>
          <div class="force force--anxiety"><span class="force__label">😰 ${labels.anxiety}</span><p>${j.anxiety}</p></div>
        </div>
        <div class="jtbd-card__skills">
          <div class="skill-tags">${j.skills.map(s => `<span class="skill-tag">${s}</span>`).join('')}</div>
        </div>
      </article>
    `).join('');
  }
    ...
    container.innerHTML = cases.map((c, i) => `
      <article class="usecase-card reveal" style="--delay:${i * 80}ms">
        <div class="usecase-card__header">
          <span class="usecase-card__emoji">${c.emoji}</span>
          <h3 class="usecase-card__title">${c.title}</h3>
        </div>
        <p class="usecase-card__scenario">${c.scenario}</p>
        <div class="usecase-card__chain">
          <h4>⛓️ Skill Chain</h4>
          <div class="chain-flow">${c.chain.map((s, si) =>
            `<span class="chain-step">${s}</span>${si < c.chain.length - 1 ? '<span class="chain-arrow">→</span>' : ''}`
          ).join('')}</div>
        </div>
        <p class="usecase-card__outcome"><strong>✅ Outcome:</strong> ${c.outcome}</p>
        <div class="usecase-card__metrics">
          <div class="metric"><span class="metric__label">⏱️ Time</span><span class="metric__value">${c.timeSaved}</span></div>
          <div class="metric"><span class="metric__label">💰 Cost</span><span class="metric__value">${c.costSaved}</span></div>
        </div>
      </article>
    `).join('');
  }
  ...
  function renderSkillMap() {
    ...
    container.innerHTML = swarms.map((sw, i) => `
      <div class="swarm-group reveal" style="--delay:${i * 100}ms">
        <h3 class="swarm-group__name">${sw.name}</h3>
        <div class="swarm-group__skills">
          ${sw.skills.map(s => `<span class="swarm-skill swarm-skill--${sw.color}">${s}</span>`).join('')}
        </div>
      </div>
    `).join('');
  }
  ...
  function renderFaq() {
    ...
    container.innerHTML = questions.map((q, i) => `
      <details class="faq-item reveal" style="--delay:${i * 60}ms" ${i === 0 ? 'open' : ''}>
        <summary class="faq-item__q">${q.q}</summary>
        <p class="faq-item__a">${q.a}</p>
      </details>
    `).join('');
  }
