...
> **Note:** Mermaid support is BUILT-IN to VitePress — no extra packages needed!

### 2. Copy Docs & Premium Template

// turbo
```bash
# Copy generated docs
cp -r [project_root]/docs/*.md docs-site/
cp -r [project_root]/docs/sop docs-site/sop 2>/dev/null || true
cp -r [project_root]/docs/api docs-site/api 2>/dev/null || true

# Copy premium config and theme
cp -r ~/.gemini/antigravity/skills/cm-dockit/templates/vitepress-premium/.vitepress docs-site/.vitepress

# Rename README.md to index.md (VitePress convention)
cd docs-site
[ -f README.md ] && mv README.md index.md

# Remove internal analysis file
rm -f analysis.md _analysis.md
```
...
```bash
cd docs-site
npx vitepress build 2>&1 | tee build.log
```
