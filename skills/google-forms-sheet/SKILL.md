...
> **Kết nối form HTML → Google Sheets qua Google Apps Script.**
> Auto-retry, toast UI, contact fallback khi lỗi. Zero dependencies, works on any static site.
...
| Trigger | Action |
|---------|--------|
| User says "tạo form", "kết nối Google Sheet" | Activate this skill |
| User says "form bị lỗi", "submit không được" | Debug using Phase 4-5 |
| User says "thêm form mới vào trang" | Start from Phase 2 |
| User says "form cho website khác" | Start from Phase 1 |
...
3. **Ask user** (Socratic Gate):
   - Có bao nhiêu Google Sheet riêng biệt? (1 sheet chung hay tách?)
   - Mỗi sheet cần những cột gì?
   - Có kênh liên hệ fallback không? (Zalo, Messenger, Hotline?)
   - URL fallback contact (ví dụ: `https://zalo.me/0559669663`)
...
| 2 | course    | 1     | name, phone, goal | Sheet Khóa Học |
```
...
|--------|--------|---------------|
| Thời gian | `new Date()` — auto | ✅ Yes |
| (form fields) | `e.parameter.xxx` | From inventory |
| Nguồn trang | `e.parameter.url` | ✅ Yes |
...
| Button states | "Đang gửi..." → "Đang thử lại (X/3)..." → reset |
...
| 2 | Submit invalid phone | Toast error "Số điện thoại không hợp lệ" | |
...
| SEO for forms | `@[skills/seo-fundamentals]` |
