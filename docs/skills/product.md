---
title: Product Skills
description: CodyMaster skills for UX, planning, JTBD, CRO, design systems, and UI preview workflows.
keywords: product skills, ux master, planning, jtbd, ui preview
robots: index, follow
---

# Product Skills

| Skill | Open |
|-------|------|
| cm-booking-calendar | `skills/cm-booking-calendar/SKILL.md` |
| cm-brainstorm-idea | `skills/cm-brainstorm-idea/SKILL.md` |
| cm-cro-methodology | `skills/cm-cro-methodology/SKILL.md` |
| cm-design-system | `skills/cm-design-system/SKILL.md` |
| cm-google-form | `skills/cm-google-form/SKILL.md` |
| cm-jtbd | `skills/cm-jtbd/SKILL.md` |
| cm-planning | `skills/cm-planning/SKILL.md` |
| cm-readit | `skills/cm-readit/SKILL.md` |
| cm-ui-preview | `skills/cm-ui-preview/SKILL.md` |
| cm-ux-master | `skills/cm-ux-master/SKILL.md` |

## See also

- [TRIZ-Parallel engine](../architecture/triz-parallel-engine.md)  
- [Vibe coding guide](../getting-started/vibe-coding-guide.md)  
- [All skills](./index.md)  
