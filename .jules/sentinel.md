...
## 2024-05-24 - Fix Command Injection in tdd-gate.ts
**Vulnerability:** `execSync` used with string interpolation for `npx vitest run ${testFile}` in `src/execution/tdd-gate.ts`, creating a critical command injection risk if `testFile` contains shell metacharacters.
**Learning:** Using `execSync` to run shell commands where inputs are dynamically concatenated is a high-risk pattern.
**Prevention:** Use `execFileSync` with an arguments array to bypass shell interpretation and dynamically set the binary based on OS to maintain cross-platform execution capability.
