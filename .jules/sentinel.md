...
## 2026-08-20 - [Command Injection via execSync in TDD Gate]
**Vulnerability:** Found a critical command injection vulnerability in `src/execution/tdd-gate.ts`. The code used `execSync` with string interpolation: `execSync(\`npx vitest run ${testFile} --reporter=verbose\`)`. A crafted `testFile` string (e.g. `"; touch /tmp/pwned; "`) could execute arbitrary shell commands.
**Learning:** Using `execSync` (which spawns a shell) with string interpolation of user/external inputs is highly dangerous. Even in seemingly safe contexts like file paths, shell metacharacters can cause catastrophic code execution. Furthermore, executing `npx` via `execFileSync` on Windows is unsafe as it implicitly uses `cmd.exe`.
**Prevention:** Always use `child_process.execFileSync` with an arguments array, which passes arguments directly to the binary without shell evaluation. For Node.js CLI tools (like `vitest`), avoid `npx` and instead securely resolve the package's binary via `require.resolve('pkg/package.json')` and execute it using `process.execPath`.
