...
## 2025-05-24 - File-Based Fallback Secret Generation for Cross-Process CLI

**Vulnerability:** A hardcoded token (`dev-token-change-me`) was used as a fallback for authentication in `src/cli/commands/engineering.ts` for the local browse daemon.
**Learning:** When creating a dynamic token fallback mechanism for CLI tools that spawn servers and send requests across different node processes, generating the token purely in-memory fails because each process generates its own unique token, breaking authentication.
**Prevention:** For cross-process fallback tokens, generate the token securely (e.g., `crypto.randomBytes(16).toString('hex')`) and persist it locally (e.g., `.cm/browse_token`) with secure file permissions (`0o600`) so that both the server and the client processes can read the same dynamically generated secret.
