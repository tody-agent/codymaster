...
## 2026-05-18 - [Critical] Command Injection via vitest testFile in TDD Gate
**Vulnerability:** The TDD automated gate script (`src/execution/tdd-gate.ts`) used `execSync` with string interpolation (`npx vitest run ${testFile}`) to run tests. This exposed the system to command injection if a maliciously crafted or unsanitized file path containing shell metacharacters was passed to the script.
**Learning:** Relying on `execSync` and shell interpretation for user-provided or filesystem-derived strings is dangerous, even in internal tooling like pre-commit hooks or automated gates, as the input source is often indirectly attacker-controllable (e.g., via a cloned repo with malicious filenames).
**Prevention:** Always use `child_process.execFileSync` or `spawnSync` with an arguments array to bypass shell interpretation entirely when handling dynamic inputs. Ensure cross-platform compatibility by resolving the correct executable (e.g., `npx.cmd` on Windows vs `npx` on POSIX).
