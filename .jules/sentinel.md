...
## 2024-05-24 - Exposed Local Server Binding
**Vulnerability:** Python scripts starting an HTTP or Uvicorn server bound to the `0.0.0.0` wildcard interface, exposing the local server to the local area network instead of keeping it strictly local.
**Learning:** Hardcoding `0.0.0.0` or allowing it as a fallback in network bindings like `HTTPServer(("0.0.0.0", port))` or `uvicorn.run(host="0.0.0.0")` breaks the isolation of the local server environment, which is highly insecure for utility/MCP servers designed to run locally.
**Prevention:** Always hardcode or fallback to `127.0.0.1` for loopback interface binding for local servers unless there is a specific intention with authentication/authorization designed for external access.
