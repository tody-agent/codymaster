...
## 2024-06-25 - Secure Local Servers Binding
**Vulnerability:** Found multiple Python servers (MCP and Dashboard servers) binding to `0.0.0.0` by default. This exposes local, internal tools and endpoints to the entire network, potentially allowing unauthenticated access from other machines on the same network.
**Learning:** Development servers in Python scripts and MCP servers were defaulting to the wildcard address out of convenience, disregarding the risk of unauthorized local network access.
**Prevention:** Always default internal and development servers to `127.0.0.1` (localhost). Use environment variables (like `HOST`) to allow overriding this behavior when intentionally required (e.g., inside Docker containers).
