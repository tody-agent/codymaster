...
## 2026-06-30 - [Critical] Unauthenticated Local Network Exposure in Python Projects

**Vulnerability:** Similar to the previous issue in the Python skills, the Python HTTP servers (`projects/cm-content-factory/scripts/dashboard_server.py`) and MCP servers (`projects/cm-ux-master/mcp/server.py`) defaulted to binding to `0.0.0.0` or had `0.0.0.0` hardcoded in config files (`projects/cm-ux-master/mcp/mcp-config.json`). This exposed the unauthenticated dashboards and internal design tool intelligence to the entire local network.
**Learning:** The risk of `0.0.0.0` bindings also extended to the `projects/` directory which appears to be a mirror or similar setup to the `skills/` directory.
**Prevention:** Always explicitly bind local-only servers to `127.0.0.1`. When initializing Python apps, use `("127.0.0.1", port)` or `127.0.0.1` as the default host parameter. Regularly audit all network socket creation code for missing explicit localhost binding.
