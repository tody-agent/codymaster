...
## 2024-06-24 - Rate Limiter for Dashboard API
**Vulnerability:** The local dashboard API lacked rate limiting on sensitive API endpoints, allowing potential local DoS attacks.
**Learning:** Even local APIs must be guarded against high-volume attacks, specially if they hold sensitive data or operations. Use established libraries like `express-rate-limit`. Do not add `@types/*` for libraries that already ship with types to avoid technical debt.
**Prevention:** Always add rate limits for APIs handling actions, and check for built-in type definitions for new dependencies.
