...
## 2024-05-18 - [Critical] Command Injection via execSync with Untrusted Input

**Vulnerability:** The test execution gate (`src/execution/tdd-gate.ts`) used `execSync(\`npx vitest run ${testFile} --reporter=verbose\`)`. By passing a file path directly into the command string without sanitization, it created a command injection vulnerability where a malicious filename could execute arbitrary shell commands.
**Learning:** Using string interpolation with `exec` or `execSync` is inherently unsafe when handling any dynamic input, even file paths.
**Prevention:** Always use `execFileSync` or `spawnSync` with an arguments array (e.g., `execFileSync('npx', ['vitest', 'run', testFile])`) to bypass shell interpretation and safely pass untrusted strings as distinct arguments. For cross-platform support with Node.js binaries, dynamically determine the executable extension (e.g., `process.platform === 'win32' ? 'npx.cmd' : 'npx'`).
