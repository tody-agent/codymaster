...
## 2025-02-15 - Command Injection in Execution Gate
**Vulnerability:** The `src/execution/tdd-gate.ts` script used `execSync` with string interpolation for executing a dynamic test file path (`execSync(\`npx vitest run ${testFile}\`)`). This exposed a command injection risk if a maliciously crafted test filename was processed. Furthermore, the command execution implicitly spawned a shell, which relies on `cmd.exe` on Windows and poses additional cross-platform security risks.
**Learning:** Build verification scripts handling dynamic file paths are susceptible to command injection just like network endpoints. Implicitly spawning shells, especially through string interpolation in child process functions like `exec` and `execSync`, dramatically widens the attack surface for such vulnerabilities.
**Prevention:** Avoid string interpolation for child process execution entirely. Use arguments arrays with safe execution contexts like `execFileSync`, and directly resolve JavaScript executable entry points (e.g., parsing `package.json` for bin files) running them directly under the Node executable (`process.execPath`) to bypass vulnerable shell wrappers.
