...
## 2026-06-14 - [Critical] Shell Injection via execSync with Interpolation

**Vulnerability:** The `runTests` function in `src/execution/tdd-gate.ts` used `execSync(\`npx vitest run ${testFile} --reporter=verbose\`)` to execute tests. Because `testFile` was interpolated directly into a string executed by the shell, an attacker could supply a maliciously crafted file path (e.g., `testFile = 'valid.ts; rm -rf /'`) to execute arbitrary shell commands.
**Learning:** Passing user-supplied or dynamic strings into `exec` or `execSync` without using argument arrays is a classic Command Injection vector. Even if the input seems constrained (like a file path), a determined attacker or a compromised dependency could manipulate it to gain shell access.
**Prevention:** Always use `child_process.execFile` or `child_process.execFileSync` with an arguments array (e.g., `execFileSync('npx', ['vitest', 'run', testFile])`) instead of `execSync` with string interpolation. This prevents the shell from interpreting metacharacters like `;`, `|`, or `&&`.
