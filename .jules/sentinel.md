...
## 2026-05-11 - [Critical] Command Injection via execSync in TDD Gate

**Vulnerability:** The `runTests` function in `src/execution/tdd-gate.ts` used `execSync` with string interpolation (`execSync(\`npx vitest run ${testFile} --reporter=verbose\`)`). If `testFile` contained maliciously crafted shell characters (like `;`, `&`, `|`, etc.), an attacker could execute arbitrary commands in the host environment (Command Injection). This is especially risky in development tooling where file paths might originate from untrusted sources.
**Learning:** Shell interpreters process spaces and special characters. When dynamic or user-controlled input (even something as seemingly harmless as a file path) is concatenated into a shell command using `execSync` or `exec`, it opens up the possibility for command injection.
**Prevention:** Avoid `execSync` and `exec` when dealing with variable input. Instead, use `execFileSync` or `spawnSync` and pass arguments as an array. The array approach bypasses the shell completely, treating the parameters safely as literal arguments to the executable.
