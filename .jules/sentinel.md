...
## 2024-05-16 - [Critical] Command Injection via testFile in TDD Gate
**Vulnerability:** The `runTests(testFile)` function in `src/execution/tdd-gate.ts` used `execSync` with template literal string interpolation: ``execSync(`npx vitest run ${testFile} --reporter=verbose`, ...)``. An attacker who can control or influence the name of the `testFile` argument can execute arbitrary shell commands (Command Injection) because `execSync` executes the string in a shell.
**Learning:** Passing strings that include untrusted inputs (even if they are intended to be file paths) directly into a shell execution context via string interpolation is dangerous. It allows escaping out of the argument context using shell operators like `;`, `&&`, or `||`.
**Prevention:** Use `execFileSync` or `spawnSync` instead of `execSync` when executing processes. These functions take an array of arguments, completely bypassing shell interpretation and avoiding interpolation vulnerabilities.
