## 2025-04-28 - [Critical] Unauthenticated Local Network Exposure

**Vulnerability:** The local express dashboard server (`src/dashboard.ts`) used `app.listen(port)` without explicitly specifying a host. In Node.js, this defaults to binding the server to `0.0.0.0` or `::`, which exposes the unauthenticated dashboard API to the entire local network.
**Learning:** Local CLI tools or developer dashboards that lack authentication must explicitly bind to the loopback interface (`127.0.0.1` or `localhost`). Relying on default bindings risks exposing sensitive development environments, API keys, or project source code to any device on the same local network (e.g., public Wi-Fi).
**Prevention:** Always explicitly bind local-only servers to `127.0.0.1`. When initializing Express apps, use `app.listen(port, '127.0.0.1', callback)`. Regularly audit all `app.listen` or equivalent network socket creation code for missing explicit localhost binding.

## 2026-05-04 - [Critical] Unauthenticated Local Network Exposure in Python Skills

**Vulnerability:** Similar to the previous issue in the Express server, the Python HTTP servers (`skills/cm-content-factory/scripts/dashboard_server.py`) and MCP servers (`skills/cm-ux-master/mcp/server.py`) defaulted to binding to `0.0.0.0` or had `0.0.0.0` hardcoded in config files (`skills/cm-ux-master/mcp/mcp-config.json`). This exposed the unauthenticated dashboards and internal design tool intelligence to the entire local network.
**Learning:** The risk of `0.0.0.0` bindings extends beyond core services to embedded scripts, dashboard utilities, and peripheral MCP skills. If standard frameworks (like `http.server.HTTPServer` or `uvicorn`) are used without explicit `127.0.0.1` binding, they default to exposing the API/UI globally.
**Prevention:** Always default to `127.0.0.1` in environment variable configurations (`os.getenv("HOST", "127.0.0.1")`), CLI parameter defaults, and hardcoded socket definitions across all languages and frameworks. Regularly audit configurations (e.g., `mcp-config.json`) and source code for `0.0.0.0` literals.

## 2026-07-04 - [Critical] Command Injection via execSync in TDD Gate

**Vulnerability:** The `runTests` function in `src/execution/tdd-gate.ts` used `execSync` with a dynamically interpolated string (`npx vitest run ${testFile} --reporter=verbose`). If a malicious or poorly sanitized `testFile` path was provided, it could lead to arbitrary command execution on the host machine.
**Learning:** Using `execSync` or `exec` with unescaped string interpolation for commands containing user-provided input (even file paths) creates a direct command injection vector. Shell interpretation can be manipulated using special characters like `;`, `&&`, or `|`.
**Prevention:** Always use `child_process.execFileSync` or `spawnSync` with an arguments array to bypass shell interpretation entirely, ensuring that inputs are treated as arguments rather than executable commands.

## 2026-07-04 - [Critical] execFileSync and Batch File Vulnerabilities on Windows

**Vulnerability:** A previous fix for a command injection vulnerability in `src/execution/tdd-gate.ts` replaced `execSync` with `execFileSync(npxCmd, ...)` where `npxCmd` was dynamically set to `npx.cmd` on Windows. However, `child_process.execFileSync` on Windows requires spawning a shell `cmd.exe` implicitly to execute `.cmd` or `.bat` files. This implicit shell spawn re-introduces command injection vulnerabilities if the arguments are not correctly sanitized.
**Learning:** Using `execFileSync` with batch files (`.cmd`, `.bat`) on Windows is not perfectly safe from command injection because Node.js must use `cmd.exe` to execute them. If arguments contain spaces or special characters, `cmd.exe` parsing rules apply.
**Prevention:** When avoiding command injection in cross-platform tools like `vitest` or `npm`, do not use `npx.cmd` or `npm.cmd` directly with `execFileSync`. Instead, use `process.execPath` (the current Node binary) and resolve the absolute path to the JavaScript CLI entry point (e.g., `require.resolve('vitest/package.json')` then `vitest.mjs`). This guarantees execution purely within Node.js without any shell invocation, completely eliminating command injection risks.
