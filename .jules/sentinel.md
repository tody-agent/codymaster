...
## 2024-07-02 - [Critical] Command Injection via String Interpolation in child_process.execSync

**Vulnerability:** The `src/execution/tdd-gate.ts` script used `execSync` with string interpolation to construct shell commands (`execSync(\`npx vitest run ${testFile}\`)`). If the `testFile` parameter is manipulated, it could allow arbitrary shell command execution.
**Learning:** Using `execSync` or `spawnSync` with string interpolation for external inputs is a direct command injection vector.
**Prevention:** Always use `execFileSync` or `spawnSync` with an array of arguments to bypass shell interpretation entirely. When invoking commands like `npx` via `execFileSync`, remember to dynamically check `process.platform === 'win32'` to use `npx.cmd` on Windows.
