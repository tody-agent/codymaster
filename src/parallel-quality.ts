...
import type { QualityContract, ParallelTask } from './parallel-dispatch';

// ─── TRIZ Principle Reference ───────────────────────────────────────────────
//
// #3  Local Quality   → Each agent validates its OWN changes (mini-gate)
// #40 Composite       → Each agent = implementer + tester + reviewer
//
// ────────────────────────────────────────────────────────────────────────────

// ─── Types ──────────────────────────────────────────────────────────────────

export interface MiniGateResult {
  passed: boolean;
  taskId: string;
  checks: GateCheck[];
  timestamp: string;
 ...
 */
export function generateMiniGatePrompt(
  task: ParallelTask,
 ...
 */
export function compositeAgentPrompt(
  task: ParallelTask,
  project: { name: string; path: string },
  batchInfo: { batchId: string; batchSize: number; position: number }
): string {
 ...
 */
export function getQualityContractForTask(task: Task): QualityContract {
  ...
  if (task.priority === 'urgent' || task.priority === 'high') {
    return {
      ...
      mustSelfReview: true,
      maxModifiedFiles: 5,
      timeoutMs: 180_000, // 3 min — tighter deadline
    };
  }
...
export function validateAgentReport(report: {
  ...
  testsFailed: number;
  issues: string[];
}, contract: QualityContract): MiniGateResult {
