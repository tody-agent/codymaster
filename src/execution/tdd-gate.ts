...
export function runTests(testFile: string): { failures: number; output: string } {
  try {
    ...
    const pkgContent = JSON.parse(fs.readFileSync(vitestPkg, 'utf-8'));
    const binFile = typeof pkgContent.bin === 'string' ? pkgContent.bin : pkgContent.bin.vitest;
    const vitestPath = path.join(path.dirname(vitestPkg), binFile);

    const output = execFileSync(
      process.execPath,
      [vitestPath, 'run', testFile, '--reporter=verbose'],
      {
        encoding: 'utf-8',
        timeout: 30000,
