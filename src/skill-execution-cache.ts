...
export class SkillExecutionCache {
  ...
  constructor(projectPath: string) {
    this.dbPath = path.join(projectPath, '.cm', 'context.db');
  }
  ...
  cacheExecution(
    ...
    if (!taskPattern.trim() || skillChain.length === 0) return;
   ...
   */
  findCachedChain(taskDescription: string): CachedChain | null {
    const db = getDb(this.dbPath);
   ...
   */
  recordHit(taskPattern: string): void {
    const db = getDb(this.dbPath);
    db.prepare(`
      UPDATE skill_cache SET hit_count = hit_count + 1, last_hit = ? WHERE task_pattern = ?
   ...
   */
  listCachedChains(limit = 20): CachedChain[] {
    const db = getDb(this.dbPath);
    try {
      const rows = db.prepare(
        'SELECT * FROM skill_cache ORDER BY hit_count DESC, effectiveness DESC LIMIT ?'
  ...
  private findExactMatch(taskPattern: string): CachedChain | null {
    const db = getDb(this.dbPath);
    const row = db.prepare(
      'SELECT * FROM skill_cache WHERE task_pattern = ? LIMIT 1'
    ).get(taskPattern) as Record<string, unknown> | undefined;
  ...
  private rowToCachedChain(row: Record<string, unknown>): CachedChain {
    let chain: string[] = [];
    try {
