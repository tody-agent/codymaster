 ...
 */
export function scoreStepRelevance(taskTitle: string, step: ChainStep): number {
  const taskTokens = new Set(
    taskTitle.toLowerCase().split(/\W+/).filter(t => t.length > 2)
  );
...
export function selectTopSkills(
  ...
  if (mandatory.length > maxSkills) {
    process.stderr.write(
      `[CodyMaster] Chain "${chain.name}" has ${mandatory.length} mandatory steps (>${maxSkills}). ` +
